...
def main() -> None:
    ...
    with open(input.cfg, 'r') as f:
        params = toml.load(f)
    params['name'] = input.file_name
    params['tag'] = input.file_name
    params['train_file'] = root_dir+'train_'+input.file_name+'.xyz'
    params['valid_file'] = root_dir+'test_'+input.file_name+'.xyz'
    #params['test_file'] = root_dir+'test_'+input.file_name+'.xyz'
    tag = params['tag']
    args = tools.build_default_arg_parser().parse_args([f'--name={params["name"]}',f'--train_file={params["train_file"]}' ])
