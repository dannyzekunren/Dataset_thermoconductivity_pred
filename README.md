...
### Evaluation
To evaluate the models, run each script in the respective training folder to train a model from scratch. The evaluation metrics include Mean Absolute Error (MAE) and R-squared ($R^2$) on three different data splits: Random, Space Group (SPG), and Out-Of-Distribution (OOD). <mark>Remember to change the split set when evaluating different scenarios.</mark>
